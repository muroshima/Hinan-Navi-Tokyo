...
| `public/data/quake_risk.geojson` | 地震に関する地域危険度測定調査（第9回・令和4年9月公表） | 東京都(都市整備局) | CC BY 4.0 | 2026-08 | SHP（**平面直角座標系第9系・JGD2000**）の町丁目ポリゴン5,192件を経緯度へ逆変換し、建物倒壊／火災／総合の危険度ランク(1〜5)を保持。配信量を抑えるため約15m許容誤差で簡素化＋座標を小数4桁に丸め。`scripts/quake_data.py`で生成（約2.1MB） |
| `public/data/quake_grid.json` | 震度分布・液状化（令和4年度 首都直下地震等による東京の被害想定結果） | 東京都(総務局) | CC BY 4.0 | 2026-08 | **都心南部直下地震（M7.3）**のケースを採用。計測震度の50mメッシュ(約69万件・13.7MB)を**250mメッシュへ集約**（セル内最大＝安全側）し、液状化(PL値・沈下量)と同一格子に統合。`scripts/quake_data.py`で生成（約2.9万セル/約850KB） |
...
### 地震の地域危険度・想定震度・液状化（#106）

地震は水害と違って予報が出ず、「どこへ逃げるか」の判断材料が事前のリスク分布しかない。東京都が公表する2系統のデータを、クライアントが引ける軽量な形へ落とす前処理バッチ。

```bash
python3 scripts/quake_data.py   # 元データを自動DL(data-raw/quake/にキャッシュ)→ public/data/quake_risk.geojson と quake_grid.json を生成
# pyshp が必要: pip install pyshp
```

| 元データ | 取得元 | 備考 |
|---|---|---|
| 地域危険度（第9回）SHP | `https://www.toshiseibi.metro.tokyo.lg.jp/bosai/chousa_6/download/all2.zip` | zip内のファイル名が日本語(CP932)のため `unzip` は失敗する。スクリプトは `zipfile` で拡張子ごとに英名展開する |
| 計測震度 50mメッシュ | `https://www.opendata.metro.tokyo.lg.jp/soumu/0_keisokusinndo50m_tosinnnannbutyokka.csv` | 都心南部直下地震。CP932・約69万行 |
| 液状化 250mメッシュ | `https://www.opendata.metro.tokyo.lg.jp/soumu/9_ekijyouka250m_tosinnnannbutyokka.csv` | 同シナリオ。**250mメッシュ中心の経緯度を持つ**ためコード変換不要 |

- **座標変換**: 地域危険度SHPは平面直角座標系第9系。`pyproj` への依存を避け、国土地理院の換算式（ガウス・クリューゲル逆算）を `scripts/quake_data.py` に実装。原点(0,0)が(139°50′, 36°00′)へ厳密に戻ることを検証済み。
- **メッシュ統合**: 50mメッシュコード(12桁 = 3次メッシュ8桁 + 緯度方向2桁 + 経度方向2桁, 各00〜19)から中心座標を復元し、250mメッシュ（緯度7.5秒・経度11.25秒）の格子へ集約。液状化データの中心座標も同じ格子に落ちるため、1ファイルに統合できる。
- **セル内は最大値**を採る（避難判断は安全側に倒す）。
- **危険度ランクは相対評価**（東京都が順位により5区分）。そのため3指標ともランク別の町丁目数は同一になる（ランク5=85件など）。指標間の差は同一町丁目のランクを比べて確認できる。
- 想定シナリオは8種類公表されているが、**都心南部直下地震の1本のみ同梱**する。全部入れると配信量が8倍になるうえ、どのシナリオでも避難行動の判断は変わらないため。
- 出典表示: 「地震に関する地域危険度測定調査（第9回）」（東京都都市整備局）／「首都直下地震等による東京の被害想定（令和4年度）」（東京都総務局）（いずれも CC BY 4.0）。
