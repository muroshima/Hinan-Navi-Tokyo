...
- **位置情報の外部送信**: 利用者の現在地・入力した住所は、経路取得（OSRM）・地名検索（Nominatim）のため**第三者の公開サーバに送信**されます。アプリ側でこれらの座標を保存・記録することはありません（サーバーログには内部エラーのみ出力）。
- **共有URL/QR**: 家族・支援者への共有URLには現在地が含まれますが、**約100m粒度（小数3桁）に丸めて**自宅などのピンポイント特定を避けています。共有先の取り扱いには利用者側でご注意ください。
- **抜本対応**: 第三者送信を無くすには、ルーティング/ジオコーディングの自前運用が必要（[公開タイル/ルーティング本番化 #21]）。
