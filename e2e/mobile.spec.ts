...
test("素早く弾くと勢いのぶんだけ余計に閉じる", async ({ page }) => {
  // CI の runner では mouse.move 1回ごとに数百msかかり、「素早く」を作れない。
  // 勢いが乗らないので、ゆっくり動かした場合と区別がつかず必ず落ちる。
  // ローカルでは同じ距離でも ゆっくり=中 / 素早く=小 に分かれることを確認済み
  test.skip(!!process.env.CI, "CIでは mouse イベントの間隔を制御できず、勢いを再現できない");
  await page.goto("/");
  await page
    .getByPlaceholder("例）雨の日、車椅子の母と避難したい")
    .fill("大地震で火事が広がっている。足の悪い祖母と逃げたい");
