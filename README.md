...
> ⚠️ **これは審査・検証用の限定公開デプロイです（本番サービスではありません）。** 地図タイル・徒歩経路・ジオコーディングは公開/デモ用の外部エンドポイント（OpenStreetMap タイル／OSRM デモ／Nominatim）に依存しており、これらは利用ポリシー上、一般公開アプリでの常用が禁止・非推奨です。本サービスは IP 単位のレート制限＋サーバー側キャッシュで公開エンドポイントへの負荷を抑えています。**本番運用時はタイル・ルーティング・ジオコーディングの自前/商用ホスティング化が必要**です（[issue #21](https://github.com/muroshima/Hinan-Navi-Tokyo/issues/21)。規約詳細は [docs/DATA.md](docs/DATA.md)）。
