...
> ⚠️ **これは審査・検証用の限定公開デプロイです（本番サービスではありません）。** 地図タイル・徒歩経路・ジオコーディングは公開/デモ用の外部エンドポイント（OpenStreetMap タイル／OSRM デモ／Nominatim）に依存しており、これらは利用ポリシー上、一般公開アプリでの常用が禁止・非推奨です。本サービスは IP 単位のレート制限＋サーバー側キャッシュ（いずれも Cloud Run のインスタンス単位・ベストエフォート。厳密なグローバル制御ではありません）で公開エンドポイントへの負荷を抑えています。**本番運用時はタイル・ルーティング・ジオコーディングの自前/商用ホスティング化が必要**です（[issue #21](https://github.com/muroshima/Hinan-Navi-Tokyo/issues/21)。規約詳細は [docs/DATA.md](docs/DATA.md)）。
...
- **地震ユースケース**: 地域危険度（町丁目・建物倒壊/火災/総合のランク1〜5）と想定震度・液状化（250mメッシュ）をスコアと地図に反映。延焼が迫る局面では**屋外の広域避難場所を持ち上げる**（水害と逆の判断）、液状化は車椅子・要介護ほど重く減点、経路の延焼リスクも警告。マイ・タイムラインは地震のとき**発災起点**（0〜3分／3〜10分／…）に切り替わり、外出中なら**帰宅困難者モード**で一時滞在施設を先に提示
- トイレ設備の近傍紐づけ（おむつ替え/オストメイト/大型ベッド/非常用ボタン）
- 生活継続レイヤー（災害時給水ステーション／公衆Wi-Fi／都営バス停）の重ね表示
- **スマホ最適化**: 地図を全画面に保ち、操作パネルはドラッグできるボトムシート（小/中/大の3段階）。検索すると自動で中段まで上がり入力欄は畳まれて避難先が先頭に来る。結果カードから地図の該当地点へ寄せられる。タップ領域44px以上、`dvh` と `safe-area` 対応、視差の軽減（`prefers-reduced-motion`）を尊重
...
# 地震データ（地域危険度・想定震度・液状化）は元データを自動DLして生成
#   pip install pyshp が必要
python3 scripts/quake_data.py
