...
「その人が行ける順」は、どの単独オープンデータにも存在しません。**避難所 × 災害種別フラグ × バリアフリー設備 × ハザード × 高齢化率 × 建物高さ × 距離**を、当事者属性・地理・災害で掛け合わせて初めて生成される情報です（例: 洪水想定では「最寄り300mのA小学校（浸水域・洪水非対応）」を降格し、「800m先で洪水対応・スロープ有のB施設」を根拠付きで1位に）。この**順位の逆転と根拠**が、掛け合わせでのみ生まれる新たな価値です。詳細は [docs/positioning.md](docs/positioning.md) の「データの掛け合わせで生まれる『行ける順』」節を参照。
