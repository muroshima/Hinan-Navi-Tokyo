...
「その人が行ける順」は、どの単独オープンデータにも存在しません。ランキングのスコアは **避難所 × 災害種別フラグ × バリアフリー設備（施設属性＋近傍トイレ）× 当事者属性 × 距離** を掛け合わせて算出します（例: 洪水想定では「最寄り300mのA小学校＝災害種別フラグで洪水非対応・1階/EV情報なし」を降格し、「600m先で洪水対応・スロープ/車椅子トイレ有のB施設」を根拠付きで上位に）。さらに **ハザードタイル・高齢化率（町丁目）・建物高さ（垂直避難）・給水/Wi-Fi・OSRM実徒歩距離** を地図に重ねて意思決定を支援します（これらは文脈表示でスコアには入れません）。この**順位の逆転と根拠**が、掛け合わせでのみ生まれる新たな価値です。詳細は [docs/positioning.md](docs/positioning.md) の「データの掛け合わせで生まれる『行ける順』」節を参照。
